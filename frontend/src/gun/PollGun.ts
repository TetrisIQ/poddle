...
export interface def {
  pollPaths:
    ...
    | "note"
    | "settings.fcfs"
    | "settings.threeOptions"
    | "settings.deadline"
    | "settings.voteLimit"
    | "settings.voteLimitAmount"
    | "deadline"
    | "options";
}
...
export class PollGun {
  ...
  async createPoll(poll: Poll) {
    ...
    await gun.encryptAndSave(
      poll.id,
      poll.password,
      poll.settings.voteLimit,
      "settings.voteLimit"
    );
    await gun.encryptAndSave(
      poll.id,
      poll.password,
      poll.settings.treeOptions,
      "settings.threeOptions"
    );
    await gun.encryptAndSave(
      poll.id,
      poll.password,
      poll.settings.voteLimitAmount,
      "settings.voteLimitAmount"
    );
    await gun.encryptAndSave(
      poll.id,
  ...
  async addParticipant(participant: Participant, id: string, password: string) {
    if (participant !== undefined) {
      const options = await SEA.encrypt(
        JSON.stringify(participant.chosenOptions),
        password
      );
        ...
        .put(
          ...
          function (ack) {
            if (ack.err) {
              ...
              console.error(
                "Cannot save: (ack, participant, name, options)",
                ack,
                participant,
                name,
                options
    ...
    gun.db.get(`poll/${id}/participants`).on((data, key) => {
      if (Object.keys(data).length - 1 !== this.poll.participants.length) {
        for (let valueKey in data) {
          if (valueKey !== "_") {
            gun.db
              .get(`poll/${id}/participants/${valueKey}`)
              .on(async (data, key) => {
                const name = (await SEA.decrypt(data.name, password)) as string;
                const options = (await SEA.decrypt(
                  data.chosenOptions,
                  password
                )) as Array<{ id: number; value: "yes" | "no" | "ifNeededBe" }>;
                currentPoll.update((arr) => {
                  if (
                    arr.participants.find((p) => p.randomKey === valueKey) !==
                    undefined
                  ) {
                    const existingParticipant = arr.participants.find(
                      (p) => p.randomKey === valueKey
                    );
                    existingParticipant.chosenOptions = options;
                  } else {
                    arr.participants.push(
                      new Participant(name, false, undefined, options, valueKey)
                    );
                  }
                  arr = arr;
                  return arr;
