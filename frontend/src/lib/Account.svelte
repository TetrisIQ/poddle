<script lang="ts">
    ...
    function deleteAccount() {
        if (window.confirm("Are you sure you want to delete this account? This cannot be undone.")) {
            users.delete(lstore.getUsername(), lstore.getPassword());
        }
    }
...
{:else }
    <div class="flex items-center justify-center">
        <div class="w-full max-w-md">
            <div class="">
                <ul class="">
                    <li class="p-2">Login to sync MyPolls and your settings across different browsers.</li>
                    <li class="p-2">You don't need an Email address for your account so you cannot reset your password if you lose it. There is no way you can restore it.</li>
                    <li class="p-2">Choose whistle!</li>
                    <li class="p-2">User Accounts are still in development any feedback is welcome!</li>
                </ul>
            </div>
            <form on:submit|preventDefault={login} class="bg-white shadow-lg rounded px-12 pt-6 pb-8 mb-4">
                ...
                <div class="flex items-center justify-between">
                    <button class="px-4 py-2 rounded text-white inline-block shadow-lg bg-indigo-500 hover:bg-blue-600 focus:bg-blue-700"
                            type="submit">Sign In
                    </button>
