<script lang="ts">
  ...
  function copyLink() {
    navigator.clipboard.writeText(pollUrl as string);
    linkCopyed = true;
  }
  ...
  <div class="bg-gray-100 w-full my-4 p-4 rounded-xl md:w-1/2 lg:w-1/3">
    ...
    <div class="my-4">
      ...
      >
        ...
        <button
          on:click={() => copyLink()}
          class="text-white rounded text-sm py-2 px-5 mr-2 hover:bg-indigo-600 {linkCopyed
            ? 'bg-indigo-600'
            : 'bg-indigo-500'}"
        >
          Copy
