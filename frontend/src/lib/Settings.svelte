...
<br>
<h1 class="text-2xl dark:text-white">Settings</h1>
<!-- Dark mode is removed for now -->
<!--<div class="mt-6 grid grid-flow-col justify-between auto-cols-max dark:text-white">
    <p class="text-xl py-4">Toggle dark mode</p>
    <ToggleDarkMode/>
</div>-->

<p class="text-xl mb-2 mt-6 dark:text-white">Change your relay servers</p>
<table class="w-full">
    <tr class="w-1/2 dark:bg-indigo-700 dark:text-white bg-indigo-300">
    ...
    {#if $allRelays.length > 0}
        {#each $allRelays as relay , id}
            <tr class="{id % 2 === 0} bg-gray-100 dark:bg-gray-500 dark:text-white">
...
<div class="flex py-4">
    <button on:click={() => updateRelayServer()}
            class="inline-flex dark:text-white items-center dark:bg-gray-800 bg-gray-100 border-0 py-1 px-3 focus:outline-none dark:hover:bg-gray-700 hover:bg-gray-200 rounded text-base ml-auto mt-4 md:mt-0">
...
<div class="mt-6 grid grid-flow-col justify-between auto-cols-max">
    <p class="text-xl dark:text-white py-4">Delete browser data</p>
