    ...
    {:else }
        <div class="mx-auto max-w-4xl rounded overflow-hidden shadow-lg">
            <div class="px-6 py-4">
                ...
                <div class="font-normal mb-8">by {$currentPoll.creatorName}
                    • {formatCreated($currentPoll.created)}</div>
                <div class="space-y-8">
                            ...
                            {#each $currentPoll.options as option}
                                {#if option.type === "week"}
                                    <th class="border border-slate-300 py-2 px-5">
                                        ...
                                        <p>{getStartFromTimeSlot(option.option)} - {getEndFromTimeSlot(option.option)}</p>
                                    </th>
