<script lang="ts">
  ...
  let pollOptions = [new Option(optionsIdCount++), new Option(optionsIdCount++)];
  ...
  function next() {
    if (selectedTab === 'text') {
      ...
      $events.forEach((o) =>
        weekOptions.push(new Option(Number(o.id), o.title + ' | ' + o.start + ' | ' + o.end, 'week'))
      );
...
>
  ...
  <div class="px-6 py-4">
    ...
    <nav class="flex justify-center flex-col sm:flex-row">
      <button
        on:click={() => window.alert('Only Text is Supported for now')}
        class="text-gray-600 py-4 px-6 block hover:text-blue-500 focus:outline-none {selectedTab === 'month'
      ...
      <button
        ...
        class="text-gray-600 py-4 px-6 block hover:text-blue-500 focus:outline-none {selectedTab === 'week'
          ? 'text-blue-500 border-b-2 font-medium border-blue-500'
          : ''}"
      >
        Week
      ...
      <button
        ...
        class="text-gray-600 py-4 px-6 block hover:text-blue-500 focus:outline-none {selectedTab === 'text'
          ? 'text-blue-500 border-b-2 font-medium border-blue-500'
          : ''}"
      >
        Text
