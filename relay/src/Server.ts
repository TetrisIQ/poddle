...
let otherPeers = process.env.OTHER_PEERS || ["https://gun.tetrisiq.de/gun"];

console.log(myAddress, myCountry, myName, connectToOtherPeers, otherPeers);

if (myAddress === undefined) {
  throw new Error("Define your public Address");
}
...
app.post(
  ...
  async (req: Request, res: Response): Promise<Response> => {
    ...
    // @ts-ignore
    return res.status(200).send(JSON.stringify(counter));
  }
);
...
app.get(
  ...
  async (req: Request, res: Response): Promise<Response> => {
    ...
    for (let valueKey in value) {
      if (valueKey !== "_") {
        // @ts-ignore
        const counter: number = await gun
          .get("poll-counter")
          .get(valueKey)
...
gun
  .get("peers")
  .get(myAddress)
  .put({ status: "UP", name: myName, country: myCountry });
