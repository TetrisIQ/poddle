<script>
	...
	const createStyle = ({
		...
		const animation = tweened(reverse ? to : from, { duration, delay, easing });
		...
		return { play: () => animation.set(to), reverse: () => animation.set(from) };
	};
	let self,
		canChange = true,
		changeBg = false,
	...
	const animationOptions = { to: 100, duration, easing: sineInOut, reverse: checked };
	const borderAnimation = createStyle({
		...
		css: {
			...
			'stroke-dasharray': { input: [0, 45, 75], output: [342, 154, [0, 310]] },
...
<div
	...
	style="width: {size};"
>
	<input id={labelId} type="checkbox" {disabled} on:change={handleChange} {name} />
	<svg
		class="checkbox__svg"
		style={disabled ? 'background: lightgray; border-radius: 20%; ' : ''}
